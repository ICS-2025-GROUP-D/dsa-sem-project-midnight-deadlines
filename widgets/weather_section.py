...
def create_weather_section(root):
    ...
    cloud_icon = ctk.CTkImage(
        light_image=Image.open("images/cloudy.png"), size=(200, 126)
    )
    ...
    location_icon = ctk.CTkImage(
        light_image=Image.open("images/location.png"), size=(25, 25)
    )
    location_icon_label = ctk.CTkLabel(
        frame, image=location_icon, text="", compound="center"
    )
    location_icon_label.place(x=320, y=10)

    city_label = ctk.CTkLabel(
        frame, text=get_user_location(), font=("Arial", 20), text_color="#FDFEFE"
    )
    city_label.place(x=350, y=10)

    temperature_label = ctk.CTkLabel(
        frame, text="12°c", font=("Arial", 32), text_color="#FDFEFE"
    )
    temperature_label.place(x=320, y=50)

    temp_desc_label = ctk.CTkLabel(
        frame, text="Outside temperature", font=("Arial", 17), text_color="#FDFEFE"
    )
    temp_desc_label.place(x=260, y=90)

    weather_desc_label = ctk.CTkLabel(
        frame, text="Cloudy", font=("Arial", 18), text_color="#FDFEFE"
    )
    ...
    def refresh_weather():
        ...
        date_label.configure(text=datetime.now().strftime("%A, %B %d"))

    refresh_btn = ctk.CTkButton(
        frame, text="🔃", width=30, height=30, command=refresh_weather
    )
