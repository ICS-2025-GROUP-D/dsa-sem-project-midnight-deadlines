...
class SmartHomeApp(ctk.CTk):
    def __init__(self):
        ...
        self.title_label = ctk.CTkLabel(
            self, textvariable=self.name_var, font=("Arial", 24)
        )
        ...
        self.user_dropdown = ctk.CTkOptionMenu(
            self, values=[], command=self.on_user_select
        )
        self.user_dropdown.place(x=410, y=20)

        self.add_button = ctk.CTkButton(
            self, text="Add", command=self.add_user, width=60
        )
        self.add_button.place(x=590, y=20)

        self.update_button = ctk.CTkButton(
            self, text="Update", command=self.update_user, width=60
        )
        self.update_button.place(x=660, y=20)

        self.delete_button = ctk.CTkButton(
            self, text="Delete", command=self.delete_user, width=60
        )
